CLASS lsc_zkk_travel_root DEFINITION INHERITING FROM cl_abap_behavior_saver.
...
CLASS lsc_zkk_travel_root IMPLEMENTATION.
  ...
  METHOD save_modified.
   ...
   data: travel_log_update type STANDARD TABLE OF /dmo/log_travel,
    ...
    ENDIF.
***Booking data event logic
if create-booking is NOT INITIAL.
   RAISE ENTITY EVENT ZKK_TRAVEL_ROOT~booking_created
   FROM VALUE #( for booking in create-booking ( corresponding #( booking ) ) ).
...
CLASS lhc_Travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS validheaderdata FOR VALIDATE ON SAVE
      IMPORTING keys FOR travel~validheaderdata.
  ...
  types:  t_entity_create type table for create zkk_travel_root,
              t_entity_update TYPE table for update zkk_travel_root,
              t_entity_rep type table for REPORTED zkk_travel_root,
              t_entity_err type table for FAILED zkk_travel_root.
   methods precheck_kk_reuse
        importing
...
*            ENDIF.
 ...
 data: entity type STRUCTURE FOR CREATE zkk_travel_root,
...
*            append value #( %cid = entity-%cid %key = entity-%key ) to failed-travel.
                  append value #( %cid = entity-%cid %msg = lx_number_ranges )
               to reported-travel.
    ...
    ""Step 5: assign new booking IDs to the booking entity inside each travel
            LOOP at <travel>-%target ASSIGNING FIELD-SYMBOL(<booking_wo_numbers>).
                ...
                if <mapped_booking>-BookingId is INITIAL.
                    max_booking_id += 10.
                     <mapped_booking>-%is_draft =  <mapped_booking>-%is_draft.
  ...
  data: travels type table for create zkk_travel_root\\Travel,
          bookings_cba type table for create zkk_travel_root\\Travel\_Booking,
          booksuppl_cba type table for create zkk_travel_root\\Booking\_BookingSupplement.
...
""Step 1: Data declaration
  ...
  METHOD acceptTravel.
   ""Perform the change of BO instance to change status
    MODIFY ENTITIES OF zkk_travel_root
        ENTITY travel
    ...
    READ ENTITIES OF zkk_travel_root
        ENTITY Travel
            ALL FIELDS
                WITH CORRESPONDING #( keys )
